...
exports.transactionsSent = {
	...
	async handler(request, h) {
		...
		const transactions = await transactionsRepository.findAllBySender(wallet.publicKey, {
			...request.params,
			...utils.paginate(request),
		})
...
exports.transactionsReceived = {
	...
	async handler(request, h) {
		...
		const transactions = await transactionsRepository.findAllByRecipient(wallet.address, {
			...request.params,
			...utils.paginate(request),
		})
...
exports.votes = {
	...
	async handler(request, h) {
		...
		const transactions = await transactionsRepository.allVotesBySender(wallet.publicKey, {
			...request.params,
			...utils.paginate(request),
		})
