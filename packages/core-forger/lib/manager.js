...
module.exports = class ForgerManager {
	...
	async __monitor(round) {
		try {
			...
			logger.info(
				...
				round ? round.current : '',
				'Height:',
				round ? round.lastBlock.height.toLocaleString() : '',
			)
	...
	async __getTransactionsForForging() {
		...
		if (isEmpty(response)) {
			...
			logger.debug(
				`Received ${transactions.length} transactions from the pool containing ${
					response.poolSize
				} :money_with_wings:`,
			)
		}
	...
	__analyseNetworkState(networkState, currentForger) {
		...
		if (
			...
			return badState(
				networkState,
				`Possible double forging for delegate: ${usernames} (${currentForger.publicKey}).`,
			)
		}
