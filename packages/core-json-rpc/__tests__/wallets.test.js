...
beforeEach(async () => {
	...
	axiosMock
		...
		.reply(() => [
			200,
			{ success: true, peers: [{ status: 'OK', ip: peerMock.ip, port: 4002, height: 5, delay: 8 }] },
			peerMock.headers,
		])
	...
	describe('POST wallets.info', () => {
		...
		it('should fail to get information about the given wallet', async () => {
			axiosMock
				...
				.reply(() => [
					404,
					{ error: { code: 404, message: 'Wallet AUDud8tvyVZa67p3QY7XPRUTjRGnWQQ9Xv could not be found.' } },
					peerMock.headers,
				])
