	...
	async handler(request, h) {
		try {
			...
			const returnTrx = transactionIds.map(
				(transaction, i) => (transactionIds[i] = transactions.find(tx2 => tx2.id === transactionIds[i])),
			)
...
exports.postTransactions = {
	...
	async handler(request, h) {
		...
		if (guard.hasAny('accept')) {
			logger.info(
				`Accepted ${guard.accept.length} transactions from ${request.payload.transactions.length} received`,
			)
			...
			transactionPool.memory.forget(guard.getIds('accept')).forget(guard.getIds('excess'))
		}

		if (!request.payload.isBroadCasted && guard.hasAny('broadcast')) {
			await container.resolvePlugin('p2p').broadcastTransactions(guard.broadcast)
		}
...
exports.getBlocks = {
	...
	async handler(request, h) {
		try {
			...
			logger.info(
				`${requestIp.getClientIp(request)} has downloaded ${blocks.length} blocks from height ${
					request.query.lastBlockHeight
				}`,
			)
