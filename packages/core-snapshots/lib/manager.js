...
module.exports = class SnapshotManager {
	...
	async importData(options) {
		...
		logger.info(
			`Import from folder ${params.meta.folder} completed. Last block in database: ${lastBlock.height} :+1:`,
		)
	...
	async rollbackChain(height) {
		...
		logger.info(
			`Rolling back chain to last finished round ${newLastBlock.height / maxDelegates} with last block height ${
				newLastBlock.height
			}`,
		)
