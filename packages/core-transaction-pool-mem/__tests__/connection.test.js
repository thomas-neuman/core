	...
	describe('getTransactionsForForging', () => {
		...
		it('should not accept transaction with amount > wallet balance', async () => {
			const transactions = [
				...
				// the same sender.
				generateTransfer('testnet', delegatesSecrets[0], mockData.dummy1.recipientId, 1, 1)[0],
				// Valid, only this transaction will classify for forging.
				generateTransfer('testnet', delegatesSecrets[1], mockData.dummy1.recipientId, 1, 1)[0],
			]
