...
class ConfigManager {
	...
	buildFees() {
		Object.keys(TRANSACTION_TYPES).forEach(type =>
			feeManager.set(TRANSACTION_TYPES[type], this.getConstant('fees')[_.camelCase(type)]),
		)
	}
	...
	buildDynamicOffsets() {
		if (this.getConstant('dynamicOffsets')) {
			Object.keys(TRANSACTION_TYPES).forEach(type =>
				dynamicFeeManager.set(TRANSACTION_TYPES[type], this.getConstant('dynamicOffsets')[_.camelCase(type)]),
			)
		}
	}
}
